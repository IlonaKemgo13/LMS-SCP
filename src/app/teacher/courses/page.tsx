...
export default function TeacherCoursesPage() {
  ...
  async function fetchDashboardData(teacherId: string) {
    ...
    const [{ data: coursesData, error: coursesError }, { count: announcementsCount, error: announcementsError }] =
      await Promise.all([
        supabase
          .from("courses")
    ...
    if (announcementsError) {
      toast.error("Failed to load announcements count.")
      setLoading(false)
      return
    }
    ...
    <section className="w-full space-y-8">
      ...
      <div className="rounded-3xl bg-linear-to-r from-indigo-600 via-purple-600 to-fuchsia-600 p-8 text-white shadow-xl">
        ...
        <p className="mt-3 max-w-2xl text-white/80">
          Access and manage all assigned courses, monitor course activity, and organize teaching
      ...
      <div className="rounded-3xl bg-white shadow-sm">
        <div className="border-b p-6">
          ...
          <p className="mt-1 text-gray-500">Courses currently assigned to this lecturer.</p>
        </div>
        ...
        <div className="divide-y">
          {loading ? (
            ...
            <div className="p-6 text-sm text-gray-500">No courses assigned yet.</div>
          )}
        </div>
      </div>
    </section>
